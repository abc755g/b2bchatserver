...
### Если на сервере уже есть другой сайт

Чату нужны порты 80 и 443, и по умолчанию он занимает их на всех интерфейсах.
Когда рядом работает другой сервис, добавьте серверу второй IP-адрес и
привяжите чат к нему — установщик спросит об этом сам, если увидит несколько
адресов или занятый 80-й порт. При прямом запуске то же самое делает флаг:

```bash
./start.sh --domain chat.company.ru --bind-ip 203.0.113.10 ...
```

A-запись домена должна указывать на этот же адрес.
...
Все команды запускаются из директории `/srv/b2b-chat`:

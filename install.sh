...
# Matrix-адрес устроен как @логин:домен, поэтому в логине нельзя ни «@», ни
# двоеточия, ни заглавных букв. Спецификация разрешает только a-z 0-9 . _ = - / +
# Без этой проверки установщик молча уходил дальше с невалидным логином, создание
# пользователя падало, а на экране всё равно печаталось «Что делать дальше».
while true; do
    ADMIN_USER=$(ask "Укажите логин администратора" "$(prev_val INSTALL_ADMIN_USER 'admin')")
    if [ -z "$ADMIN_USER" ]; then
        warn "Логин не может быть пустым."
    elif ! printf '%s' "$ADMIN_USER" | grep -qE '^[a-z0-9._=/+-]+$'; then
        warn "Недопустимый логин: ${ADMIN_USER}"
        warn "Адрес пользователя выглядит как @логин:${SERVER_NAME:-домен}, поэтому «@», двоеточия,"
        warn "пробелы и заглавные буквы использовать нельзя. Разрешены: a-z 0-9 . _ = - / +"
        warn "Почтовый адрес логином быть не может — возьмите часть до «@», например ${ADMIN_USER%%@*}"
    else
        break
    fi
done
